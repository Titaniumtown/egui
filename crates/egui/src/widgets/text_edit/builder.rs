...
impl<'t> TextEdit<'t> {
    ...
    /// When enabled, show the hint text after the current text input (as a forward hint).
    #[inline]
    pub fn hint_forward(mut self, hint_forward: bool) -> Self {
    ...
    fn show_content(self, ui: &mut Ui) -> TextEditOutput {
        ...
        if ui.is_rect_visible(rect) {
            ...
            if (hint_forward || text_is_empty) && !hint_text.is_empty() {
                ...
                let hint_text_font_id = hint_text_font.unwrap_or(font_id.clone().into());
                let hint_galley = if multiline {
                ...
                let rows = hint_galley.rows.len();

                let vec_offset = if hint_forward && !text_is_empty {
                    let gallery_offset_x: f32 = ui.fonts_mut(|fonts| {
                        prev_text
                            .chars()
                ...
                let hint_galley_pos = align
                    .align_size_within_rect(hint_galley.size(), rect)
                    .intersect(rect)
                    .min;
                painter.galley(hint_galley_pos + vec_offset, hint_galley, hint_text_color);
            }
